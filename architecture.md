...
|  |  (entry point) |    |  - initializes all services           |    |
...
+------+------+          +--------+---------+          +-------+-------+
       ...
       |                          |   'tool_call',             |
       |                          |   toolName:                |
       |  {type:'agentMessage',   |   'get_tree_manifest'})   |
       |   message:{...}}         | <------------------------- |
       | <----------------------- |                            |
       |                          |                            |
       ...
       |                          |  onMessage({type:          |
       |                          |   'assistant',             |
       |  {type:'agentMessage',   |   content:'Done!...'})    |
       |   message:{...}}         | <------------------------- |
       | <----------------------- |                            |
       |                          |                            |
       |                          |  onDone()                  |
...
This is the key architectural innovation. Instead of pasting
full file contents into every prompt:
...
--------------------------------
  Turn 1: "Manifest of 50 files with 8-char hashes (500 tokens)"
...
project/
|-- src/                                     +-----------------------------+
|   |-- index.ts  -------- hash -------->    | Hash: a3f8c2d1...          |
|   |                                        | Content: "import * as..." |
|   |                                        | Size: 1234                 |
|   |-- utils.ts  -------- hash -------->    |                            |
|   |                                        | Hash: b7e9f3a0...          |
|   +-- app.ts    -------- hash -------->    | Content: "export class.." |
|                                            | Size: 567                  |
|-- package.json  -------- hash -------->    |                            |
|                                            | Hash: c1d2e3f4...          |
+-- tsconfig.json -------- hash -------->    | Content: '{"name":...'    |
                                             | Size: 890                  |
                                             +-----------------------------+
...
Note: Values are 8-char short hashes, NOT file content.
...
services/workspaceTree.ts                        services/agentLoop.ts
     |                                                    |
     |  exports: WorkspaceTree,                           |  imports:
     |  Snapshot                                          |  GeminiClient,
     |                                                    |  ContextPacker,
     +---------------+                                   |  ToolDef
     |               |                                   |
     v               v                                   |
...
|                                          |
|  +------------------------------------+  |
|  | [tree] get_tree_manifest       [>] |  |  <-- Tool card (collapsed)
|  +------------------------------------+  |
|                                          |
|  +------------------------------------+  |
|  | [blob] get_blob                [v] |  |  <-- Tool card (expanded)
|  |  +------------------------------+  |  |
|  |  | PARAMETERS                    |  |  |
|  |  | { "hash": "b7e9f3a0" }       |  |  |
|  |  +------------------------------+  |  |
...
|                                          |
|  +------------------------------------+  |
|  | [edit] edit_file               [>] |  |  <-- Another tool card
|  +------------------------------------+  |
|                                          |
|  +------------------------------------+  |
...
+------------------------------------------+
|  (*) Agent is thinking...                |  <-- Status (while running)
+------------------------------------------+
|  +------------------------+ +------+     |
|  | Ask Tech Boss...       | | Send |     |  <-- Input area
|  +------------------------+ +------+     |
+------------------------------------------+
```
